...
public class Controlador {
    ...
    public String ingresarSubmit(@ModelAttribute Usuario vendedor) {
    	System.out.println("Usuario y contraseña ingresados: "+vendedor.getUser()+" "+vendedor.getPassword());
    	if (sv.validarIngreso(vendedor.getUser(), vendedor.getPassword())) {
    		System.out.println("Ingreso validado!");
    		return "ingresoOK";}else {System.out.println("Ingreso no validado :(");return "ingresoMal";}
    ...
    public String buscarSubmit(@ModelAttribute Busqueda search, Model model) {
    	...
    	System.out.println(search.getResultado());//para ver si lo setea bien--->SI, ACA LLEGA LA RESPUESTA BIEN
    	...
    	model.addAttribute("FIN", search.getResultado());
    	return ("FinBusqueda");
    }
